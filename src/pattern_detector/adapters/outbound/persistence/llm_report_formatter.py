"""Token-Efficient LLM / AI Prompt Context Formatter for Mojo."""
...
class LlmReportFormatter:
    ...
    def format_scan_report(self, report: DetectionReport) -> str:
        ...
        lines: list[str] = [
            '<codebase_architecture_analysis language="mojo">',
        ...
        if patterns:
            lines.append("    <mojo_design_patterns_and_simd_acceleration>")
            for d in patterns:
        ...
        if hazards:
            lines.append("    <memory_safety_and_performance_hazards>")
            for v in hazards:
