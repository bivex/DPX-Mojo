"""Interactive HTML Observability HUD Formatter for Mojo."""
...
CATEGORY_COLORS: dict[PatternCategory, dict[str, str]] = {
    PatternCategory.MOJO_IDIOMATIC: {
        "text": "#f97316",
        "bg": "#7c2d1244",
        "border": "#ea580c",
        "accent": "#fb923c",
        "name": "Mojo Idiomatic & Ownership",
    },
    PatternCategory.SIMD_HARDWARE_ACCELERATION: {
        "text": "#ef4444",
        "bg": "#7f1d1d44",
        "border": "#dc2626",
        "accent": "#f87171",
...
class HtmlReportFormatter(ReportFormatterPort):
    """Renders standalone, responsive HTML dashboard for Mojo DetectionReport with AI Prompt Generator."""
    ...
    def format(self, report: DetectionReport, verbose: bool = False) -> str:
        ...
        for idx, det in enumerate(report.detections, 1):
            ...
            finding_prompt = (
                f"Refactor and optimize the following Mojo architectural finding / kernel pattern:\n"
                ...
                f"Please provide an idiomatic Mojo 24.x - 25.x+ high-performance refactoring leveraging SIMD vectorization, explicit ownership (borrowed/inout/owned), and zero-cost compile-time parameters."
            )
...
<head>
    ...
    <title>Pattern Scanner Report - DPX-Mojo Architecture HUD - {html.escape(report.project_path or "Codebase")}</title>
    <style>
        :root {{
            --bg: #0d1117;
            --card-bg: #161b22;
        ...
        * {{ box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, monospace, sans-serif; }}
        ...
        .btn-ai-context {{
            background: linear-gradient(135deg, #ff4500 0%, #ff6b35 50%, #e63946 100%);
            color: #ffffff;
            border: none;
            padding: 10px 18px;
            border-radius: 8px;
        ...
        .btn-all {{ background: #21262d; color: #f0f6fc; border-color: #30363d; }}
        ...
        .search-bar:focus {{ border-color: var(--mojo-orange); }}
        ...
        .pattern-card:hover {{ border-color: #ff450088; box-shadow: 0 4px 20px rgba(0,0,0,0.3); }}
        ...
        .card-index {{ color: #6e7681; font-weight: 700; font-size: 13px; }}
        ...
        code {{ background: #090d13; padding: 3px 6px; border-radius: 4px; font-family: ui-monospace, SFMono-Regular, Menlo, monospace; color: #ff6b35; border: 1px solid #30363d; }}
        ...
        .location-tag {{ color: #ff6b35; font-size: 11px; margin-left: 6px; }}
...
<body>
    <div class="container">
        <header>
            <div>
                <h1>🔥 DPX-Mojo Architecture & SIMD Observability HUD</h1>
                <div class="subtitle">SIMD Vectorization, Ownership, Memory Safety, GoF 23 & AI Acceleration Engine • Target: <code>{html.escape(report.project_path or "Target Repository")}</code></div>
            </div>
        ...
        <input type="text" id="searchInput" class="search-bar" placeholder="🔎 Instant search by pattern name, category, target type, or rule (e.g. simd, ownership, trait, pointer, tile)...">
    ...
    <div id="promptModal" class="modal-overlay" onclick="closeModalOnOverlay(event)">
        <div class="modal-box">
            <div class="modal-header">
                <div class="modal-title">🤖 AI Architect Context Prompt (Mojo)</div>
                <button class="modal-close" onclick="closePromptModal()">&times;</button>
            </div>
            <div class="modal-body">
                <p style="font-size: 13px; color: #8b949e; margin-bottom: 12px;">Structured XML/Markdown context for LLMs (Claude, GPT-4, Gemini) including SIMD vectorization, ownership borrow checker, and hardware acceleration analysis.</p>
    ...
    <script>
        ...
        filterBtns.forEach(btn => {{
            btn.addEventListener('click', () => {{
                ...
                btn.style.outline = '2px solid #ff4500';
                filterCards();
            }});
        }});
    </script>
