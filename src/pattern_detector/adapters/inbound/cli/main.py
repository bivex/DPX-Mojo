"""Typer and Rich CLI interface for DPX-Mojo."""
...
app = typer.Typer(
    name="dpx-mojo",
    help="🔥 Modern Mojo Static Analyzer: SIMD Vectorization, Ownership, Memory Safety, GoF 23 & AI Hardware Acceleration Architectural Pattern Detector.",
...
def scan(
    path: Annotated[
        str,
        typer.Argument(
            help="Path to a Mojo source file (.mojo, .🔥) or directory containing a Mojo package/project.",
        ),
    ] = ".",
    ...
    pattern: Annotated[
        ...
        typer.Option(
            "--pattern",
            "-p",
            help="Filter for specific pattern type(s) (e.g. 'simd_vectorization_kernel', 'explicit_ownership_borrow', 'unchecked_unsafe_pointer').",
        ),
    ] = None,
    ...
    exclude: Annotated[
        ...
        typer.Option(
            "--exclude",
            "-e",
            help="Directory name(s) to exclude from scanning (e.g. -e build -e .modular).",
        ),
    ] = None,
    verbose: Annotated[
        bool,
    ...
    """Scan a Mojo codebase for SIMD vectorization, ownership, GoF patterns, and memory safety hazards."""
...
@app.command()
def rules() -> None:
    """List all available Mojo detection rules and hardware acceleration checks."""
    table = Table(
        title="🔥 DPX-Mojo: Supported Pattern Rules & Quality Checks",
...
def info(
    pattern_name: Annotated[
        str,
        typer.Argument(
            help="Pattern type or name to query (e.g. 'simd_vectorization_kernel', 'explicit_ownership_borrow').",
        ),
    ],
) -> None:
    """Display deep architectural details, rationale, and advice for a specific Mojo pattern."""
    ...
    panel = Panel.fit(
        ...
        f"[bold white]Mojo Target:[/bold white] {pdef.mojo_version}\n\n"
        f"[bold white]Description:[/bold white]\n{pdef.description}\n\n"
        f"[bold white]Architecture Recommendation:[/bold white]\n{pdef.recommendation or 'Adheres to standard Mojo high-performance idiom.'}",
        title=f"🔍 Mojo Architecture Encyclopedia: {pdef.name}",
        border_style="red",
    )
    console.print(panel)
