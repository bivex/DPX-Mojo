"""Inbound driving ports for DPX-Mojo."""
...
@dataclass
class ScanOptions:
    """Configuration options for a Mojo scanning session."""

    min_confidence: float = 0.0
    enabled_patterns: list[str] = field(default_factory=list)
    file_extensions: list[str] = field(default_factory=lambda: [".mojo", ".🔥"])
...
class ScannerPort(Protocol):
    """Inbound port for scanning a target directory or Mojo source file."""
