"""Outbound driven ports for DPX-Mojo."""

from __future__ import annotations

from typing import Protocol
from pattern_detector.domain.code_model import CodeModel, MojoFile
from pattern_detector.domain.detection import DetectionReport


class SourceProviderPort(Protocol):
    """Port for discovering and loading Mojo source files (.mojo, .🔥)."""
...
class ParserPort(Protocol):
    """Port for parsing Mojo source text into CodeModel."""

    def parse_file(self, file_path: str, content: str) -> MojoFile:
        """Parse a single Mojo file into MojoFile model."""
        ...

    def parse_codebase(self, files: list[tuple[str, str]], target_path: str = "") -> CodeModel:
        """Parse multiple Mojo files into an aggregated CodeModel."""
        ...
